...
def getQuestionTokens():
    austen = [w for w in nltk.corpus.gutenberg.words('austen-emma.txt') if w not in PUNCTUATION]
    shakespeare = [w for w in nltk.corpus.gutenberg.words('shakespeare-macbeth.txt') if w not in PUNCTUATION]
    return austen, shakespeare
...
def main():
    ...
    tok1, tok2 = getQuestionTokens()
    grams1 = getEveryGram(tok1, 2, 3)
    grams2 = getEveryGram(tok2, 2, 3)
    tokDist1 = FreqDist(grams1)
    tokDist2 = FreqDist(grams2)
    mostCommon = 25
    print("%d Most Common for Austen's Emma       : %s" % (mostCommon, pformat(tokDist1.most_common(mostCommon))))
    print("%d Most Common for Shakespear's Macbeth: %s" % (mostCommon, pformat(tokDist2.most_common(mostCommon))))
    return 0
